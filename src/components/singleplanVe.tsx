...
export default function SinglePlanVe({
	username,
	profile_photo,
	token,
	userId,
	...
	const {
		...
		`${base_url}/ardilla/retail/admin/api/v1/savings/single_vault_extra_user/${userId}`,
		token
	);
	...
	useEffect(() => {
		setPlans(
			data?.data?.user.ardilla_retail_user_saving_vault_extra_wallet
				?.vault_plan
		);
		setSelectedPlan(
			data?.data?.user.ardilla_retail_user_saving_vault_extra_wallet
